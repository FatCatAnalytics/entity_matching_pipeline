...
WITH hierarchy AS (
    SELECT
        *,
        nullif(upper(trim(matched_lei)), '') AS matched_lei_norm,
        nullif(upper(trim(direct_parent_lei)), '') AS direct_parent_lei_norm,
        nullif(upper(trim(ultimate_parent_lei)), '') AS ultimate_parent_lei_norm,
        trim(regexp_replace(regexp_replace(regexp_replace(regexp_replace(lower(coalesce(submitted_name, '')), '&', ' and '), '[^a-z0-9 ]', ' '), '\\b(inc|incorporated|ltd|limited|llc|plc|corp|corporation|co|company|ag|sa|bv|nv|gmbh|pte|pty|holdings|holding|group)\\b', ' '), ' +', ' ')) AS submitted_clean_name,
...
hierarchy_blocked AS (
    SELECT
        *,
        split(submitted_clean_name, ' ')[0] AS submitted_first_token,
        substring(submitted_clean_name, 1, 4) AS submitted_prefix_4,
...
internal_client AS (
    SELECT
        ...
        internal_rating,
        business_owner,
        clean_name AS internal_clean_name
    FROM {catalog}.{schema}.internal_client_entities_clean
),
...
lei_candidates AS (
    ...
    INNER JOIN internal_client_blocked i ON i.lei_norm IS NOT NULL AND i.lei_norm = h.matched_lei_norm
    ...
    FROM hierarchy_blocked h
    INNER JOIN internal_client_blocked i ON i.lei_norm IS NOT NULL AND i.lei_norm = h.direct_parent_lei_norm
    ...
    FROM hierarchy_blocked h
    INNER JOIN internal_client_blocked i ON i.lei_norm IS NOT NULL AND i.lei_norm = h.ultimate_parent_lei_norm
),
name_candidates AS (
    SELECT h.*, i.internal_entity_id, i.legal_name AS internal_client_name, i.lei AS internal_client_lei, i.internal_rating, i.business_owner,
           'SUBMITTED_NORMALIZED_NAME_MATCH_TO_INTERNAL_CLIENT' AS internal_mapping_type, 4 AS mapping_priority, 0.94 AS internal_mapping_score
    FROM hierarchy_blocked h
    INNER JOIN internal_client_blocked i ON h.submitted_clean_name <> '' AND i.internal_clean_name <> '' AND h.submitted_clean_name = i.internal_clean_name

    UNION ALL

    SELECT h.*, i.internal_entity_id, i.legal_name AS internal_client_name, i.lei AS internal_client_lei, i.internal_rating, i.business_owner,
           'GLEIF_NORMALIZED_NAME_MATCH_TO_INTERNAL_CLIENT' AS internal_mapping_type, 5 AS mapping_priority, 0.92 AS internal_mapping_score
    FROM hierarchy_blocked h
    ...
    SELECT h.*, i.internal_entity_id, i.legal_name AS internal_client_name, i.lei AS internal_client_lei, i.internal_rating, i.business_owner,
           'DIRECT_PARENT_NORMALIZED_NAME_MATCH_TO_INTERNAL_CLIENT' AS internal_mapping_type, 6 AS mapping_priority, 0.90 AS internal_mapping_score
    FROM hierarchy_blocked h
    ...
    SELECT h.*, i.internal_entity_id, i.legal_name AS internal_client_name, i.lei AS internal_client_lei, i.internal_rating, i.business_owner,
           'ULTIMATE_PARENT_NORMALIZED_NAME_MATCH_TO_INTERNAL_CLIENT' AS internal_mapping_type, 7 AS mapping_priority, 0.88 AS internal_mapping_score
    FROM hierarchy_blocked h
...
fuzzy_candidates AS (
    SELECT h.*, i.internal_entity_id, i.legal_name AS internal_client_name, i.lei AS internal_client_lei, i.internal_rating, i.business_owner,
           'SUBMITTED_FUZZY_NAME_MATCH_TO_INTERNAL_CLIENT' AS internal_mapping_type, 8 AS mapping_priority,
           round(1.0 - (levenshtein(h.submitted_clean_name, i.internal_clean_name) / greatest(length(h.submitted_clean_name), length(i.internal_clean_name))), 4) AS internal_mapping_score
    FROM hierarchy_blocked h
    INNER JOIN internal_client_blocked i
        ON h.submitted_clean_name <> ''
       AND i.internal_clean_name <> ''
       AND (h.submitted_first_token = i.internal_first_token OR h.submitted_prefix_4 = i.internal_prefix_4)
       AND 1.0 - (levenshtein(h.submitted_clean_name, i.internal_clean_name) / greatest(length(h.submitted_clean_name), length(i.internal_clean_name))) >= 0.78

    UNION ALL

    SELECT h.*, i.internal_entity_id, i.legal_name AS internal_client_name, i.lei AS internal_client_lei, i.internal_rating, i.business_owner,
           'GLEIF_FUZZY_NAME_MATCH_TO_INTERNAL_CLIENT' AS internal_mapping_type, 9 AS mapping_priority,
    ...
    SELECT h.*, i.internal_entity_id, i.legal_name AS internal_client_name, i.lei AS internal_client_lei, i.internal_rating, i.business_owner,
           'DIRECT_PARENT_FUZZY_NAME_MATCH_TO_INTERNAL_CLIENT' AS internal_mapping_type, 10 AS mapping_priority,
    ...
    SELECT h.*, i.internal_entity_id, i.legal_name AS internal_client_name, i.lei AS internal_client_lei, i.internal_rating, i.business_owner,
           'ULTIMATE_PARENT_FUZZY_NAME_MATCH_TO_INTERNAL_CLIENT' AS internal_mapping_type, 11 AS mapping_priority,
